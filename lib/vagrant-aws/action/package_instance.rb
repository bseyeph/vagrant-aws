      ...
      class PackageInstance < Vagrant::Action::General::Package
        ...
        def call(env)
          ...
          begin
            ...
            ami_response = server.service.create_image server.id,
                                                       "#{server.tags['Name']} Package - #{Time.now.strftime('%Y%m%d-%H%M%S')}", ''
            ...
            env[:metrics]['instance_ready_time'] = Util::Timer.time do
              ...
              server.service.create_tags(@ami_id, region_config.package_tags) unless region_config.package_tags.empty?
            ...
            env[:ui].info(I18n.t('vagrant_aws.packaging_instance_complete',
                                 time_seconds: env[:metrics]['instance_ready_time'].to_i))
